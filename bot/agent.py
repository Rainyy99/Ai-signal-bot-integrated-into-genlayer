...
class TradingAgent:
    def __init__(self, config):
        self.coins = [
            c.strip() for c in config.get("coins", "BTC").split(",")
        ]
        self.timeframe        = config.get("timeframe", "15m")
        self.scan_interval    = int(config.get("scan_interval", 60))
        self.min_strength     = float(
            config.get("min_signal_strength", 65)
        )
        self.use_genlayer     = (
            config.get("use_genlayer", "false").lower() == "true"
        )
        self.contract_address = config.get(
    ...
    async def run(self):
        ...
        while self._running:
            try:
                ...
                print("Menunggu " + str(self.scan_interval) + "s...")
    ...
    async def _scan_all(self):
        ...
        print("\nScan #" + str(self._scans) + " - " + self._last)
    ...
    async def scan_and_send_single(self, coin):
        ...
        if not signal:
            print("  " + coin + ": data error")
            return
        print(
            "  " + coin + ": " +
            signal.action + " " + str(signal.strength) + "%",
            end=""
        )
    ...
    async def _scan_single(self, coin):
        try:
            ...
            if sig:
                sig.funding_rate = (
                    await self.fetcher.fetch_funding_rate(coin)
                )
                sig.open_interest = (
                    await self.fetcher.fetch_open_interest(coin)
                )
            return sig
        except Exception as e:
            print("Scan error " + coin + ": " + str(e))
            return None
    ...
    def get_status(self):
        ...
        m, s = divmod(r, 60)
        return {
            "uptime":       str(h) + "j " + str(m) + "m " + str(s) + "d",
            "total_scans":  self._scans,
            "signals_sent": self._sent,
